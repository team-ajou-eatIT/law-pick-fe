...
export function SearchResults({ searchQuery, onBack, onNewSearch }: SearchResultsProps) {
  ...
  const lawResults = [
    {
      ...
      title: "임대차보호법",
      type: "법령",
      description: "주택임대차보호법은 주택의 임대차에 관하여 민법의 특례를 규정함으로써 국민의 주거생활의 안정을 보장함을 목적으로 합니다.",
    ...
    },
    {
      ...
      type: "법령",
      description: "임대차는 당사자 일방이 상대방에게 목적물을 사용, 수익하게 할 것을 약정하고 상대방이 이에 대하여 차임을 지급할 것을 약정함으로써 그 효력이 생긴다.",
    ...
    },
    {
      ...
      title: "상가건물 임대차보호법",
      type: "법령",
      description: "상가건물의 임대차에 관하여 민법의 특례를 규정함으로써 상가건물 임대차의 안정을 보장하고 상가건물 임차인의 권익을 보호함을 목적으로 합니다.",
  ...
  const cardNewsResults = [
    {
      ...
      type: "카드뉴스",
      description: "2024년 개정된 임대차보호법의 주요 변경사항을 카드뉴스로 쉽게 설명합니다.",
      date: "2024.12.18",
      views: 1250,
      relevance: 92
    },
    {
      ...
      type: "카드뉴스",
      description: "전월세 계약 시 보증금을 안전하게 보호받는 방법을 단계별로 안내합니다.",
      date: "2024.12.10",
      views: 2100,
      relevance: 89
    }
  ...
  const billAnalysisResults = [
    {
      ...
      type: "법안 분석",
      description: "임차인 보호 강화를 위한 개정안 분석 및 예상 효과를 상세히 설명합니다.",
      proposer: "국토교통위원회",
      status: "가결",
      date: "2024.12.22",
      relevance: 94
    },
    {
      ...
      title: "전월세상한제 개선방안",
      type: "법안 분석",
      description: "전월세상한제의 현황과 개선방안에 대한 전문가 분석 보고서입니다.",
      proposer: "정책연구소",
      status: "연구중",
      date: "2024.12.15",
      relevance: 87
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <p className="text-sm text-muted-foreground">
                "<span className="font-semibold text-foreground">{searchQuery}</span>" 검색 결과 {filteredResults.length}건
              </p>
            </div>
      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        <div className="space-y-6">
          {filteredResults.map((result) => (
            <Card key={`${result.category}-${result.id}`} className="hover:shadow-lg transition-all cursor-pointer">
                ...
                <div className="flex items-start gap-4">
                  <div className="flex-shrink-0">
                    {result.type === '카드뉴스' ? (
                      ...
                      <div className={`w-16 h-16 rounded-lg flex items-center justify-center ${getCategoryColor(result.type)}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-2">
                      <Badge variant="secondary" className={getCategoryColor(result.type)}>
                        {result.type}
                      </Badge>
                      ...
                      {'status' in result && (
                        <Badge variant={result.status === '가결' ? 'default' : 'outline'}>
                          {result.status}
                        </Badge>
                      )}
                    </div>
